...
export default function BlockOverlayCopy({ header, subtext, position }) {
  ...
  return (
    <FadeOnHover className={`px-4 sm:px-6 pt-4 ${alignClass}`}>
      {header && (
        <p className={`${typeBlockOverlay.subtext} mb-2 sm:mb-4`}>{header}</p>
      )}
      {subtext && (
        <h3 className={`${typeBlockOverlay.header} ${type.mod.white} mb-4 sm:mb-8 max-w-[85%] sm:max-w-4xl ${position?.includes("right") ? "ml-auto" : position?.includes("left") ? "mr-auto" : "mx-auto"}`}>
          {subtext}
        </h3>
      )}
    </FadeOnHover>
